    ...
    if not is_admin:
        try:
            ...
            if disallowed_imports:
                ...
                return {"success": False, "error_type": "ForbiddenImportError", "error": error_message}
...
def handle_execute():
    ...
    with subprocess_lock:
        if active_subprocess_count >= MAX_CONCURRENT_SUBPROCESSES:
                    ...
                    for filename in files_to_return:
                        ...
                        if os.path.exists(filepath) and os.path.isfile(filepath):
                            if os.path.getsize(filepath) <= MAX_FILE_SIZE_BYTES:
                                ...
                                output_files[safe_filename] = base64.b64encode(content_bytes).decode('utf-8')
                            else:
                                output_files[safe_filename] = "Error: File is too large to return."

                if result["success"]:
                    response_data["status"] = "success"
                    response_data["execution_details"] = {
                        "stdout": result["stdout"], "stderr": result["stderr"], "exit_code": result["exit_code"],
                        "execution_duration_sec": result["execution_duration_sec"], "output_files": output_files
                    }
                    return jsonify(response_data), 200
                else:
                    response_data["error_details"] = {"type": result.get("error_type", "RuntimeError"), "message": result.get("error")}
                    return jsonify(response_data), 400
            except Exception as e:
